...
C
     ...
     2,PZKI=PCKI/ZCKI)
      REAL FEXUC,FEXUN,FEXUP
      CHARACTER*32 EXUC_ENV,EXUN_ENV,EXUP_ENV
      DATA FEXUC/0.5E-03/,FEXUN/1.0E-02/,FEXUP/1.0E-02/
      REAL*4 RI,TKGO,TKSO
C     Allow override of root exudation rates via environment variables
      CALL GETENV('FEXUC',EXUC_ENV)
      IF(EXUC_ENV.NE.'') READ(EXUC_ENV,*) FEXUC
      CALL GETENV('FEXUN',EXUN_ENV)
      IF(EXUN_ENV.NE.'') READ(EXUN_ENV,*) FEXUN
      CALL GETENV('FEXUP',EXUP_ENV)
      IF(EXUP_ENV.NE.'') READ(EXUP_ENV,*) FEXUP
C     REAL*16 B,C
      DO 9995 NX=NHW,NHE
      DO 9990 NY=NVN,NVS
C
